...
class TestDtypeMapper(unittest.TestCase):
    ...
    def test_strings_become_qstring(self):
        self.assertEqual(
    ...
    def test_floats_become_double(self):
        self.assertEqual(
            QgsConverter.dtype_mapper([1.5, 2.5, 3.0]),
            QtCore.QMetaType.Type.Double,
        )

    def test_bools_become_bool(self):
        self.assertEqual(
            QgsConverter.dtype_mapper([True, False, True]),
            QtCore.QMetaType.Type.Bool,
        )

    def test_numeric_strings_become_int(self):
        self.assertEqual(
            QgsConverter.dtype_mapper(['1', '2', '3']),
            QtCore.QMetaType.Type.Int,
        )

    def test_decimal_strings_become_double(self):
        self.assertEqual(
            QgsConverter.dtype_mapper(['1.5', '2.0', '3.14']),
            QtCore.QMetaType.Type.Double,
        )

    def test_all_none_becomes_qstring(self):
        self.assertEqual(
            QgsConverter.dtype_mapper([None, None, None]),
            QtCore.QMetaType.Type.QString,
        )

    def test_empty_list_becomes_qstring(self):
        self.assertEqual(
            QgsConverter.dtype_mapper([]),
            QtCore.QMetaType.Type.QString,
        )

    def test_leading_none_and_empty_are_skipped(self):
        self.assertEqual(
